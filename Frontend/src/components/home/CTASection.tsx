...
export function CTASection() {
  ...
  return (
    <section className="py-12 lg:py-28 relative overflow-hidden bg-background">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-center">
          ...
          <div className="text-left space-y-6 md:space-y-8">
            ...
            <div className="space-y-4">
              <h2 className="font-display text-3xl sm:text-5xl lg:text-6xl font-bold text-foreground tracking-tight leading-[1.1]">
          ...
          <div className="relative group perspective-1000">
            ...
            <div className="relative rounded-[2rem] bg-card border border-border p-4 sm:p-8 lg:p-10 shadow-2xl shadow-secondary/5 overflow-hidden">
              ...
              <div className="relative z-10 space-y-5 md:space-y-8">
                <div className="flex items-center gap-4 mb-1 md:mb-2">
                  <div className="p-2 md:p-3 rounded-xl bg-primary/10 border border-primary/20 shadow-inner">
                    <Calculator className="h-5 w-5 md:h-6 md:w-6 text-primary" />
                  </div>
                  <div>
                    <h3 className="text-lg md:text-xl font-bold text-card-foreground">Earnings Calculator</h3>
                    <p className="text-[10px] md:text-xs text-muted-foreground font-medium tracking-wide">ESTIMATED REVENUE</p>
                  </div>
                </div>
                ...
                <div className="bg-muted/50 p-1 rounded-lg flex items-center gap-1">
                  {[
                    ...
                    <button
                      ...
                      className={cn(
                        "flex-1 text-[10px] md:text-xs font-semibold py-1.5 px-2 rounded-md transition-all duration-300",
                        viralMode === mode.id
                    ...
                    >
                      {mode.label}
                      <span className={cn("ml-1 text-[8px] md:text-xs align-top", viralMode === mode.id ? "text-primary opacity-100" : "opacity-0")}>
                        {mode.mul}
                      </span>
                    </button>
                  ))}
                  ...
                  <div className="flex justify-between items-center text-xs md:text-sm font-bold">
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="w-14 md:w-16 h-7 md:h-8 text-right font-bold text-sm md:text-base tabular-nums border-primary/20 focus-visible:ring-primary/20 bg-background/50"
                      />
                      <span className="text-muted-foreground font-medium text-[10px] md:text-xs">notes</span>
                    </div>
                  </div>
                  <Slider
                    value={noteCount}
                ...
                <div className="space-y-3 md:space-y-4">
                  <div className="flex justify-between items-center text-xs md:text-sm font-bold">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-muted-foreground font-medium text-[10px] md:text-xs mr-1">₹</span>
                      <Input
                        type="number"
                        value={avgPrice[0]}
                        onChange={(e) => handleInputChange(setAvgPrice, e.target.value, 5000)}
                        className="w-16 md:w-20 h-7 md:h-8 text-right font-bold text-sm md:text-base tabular-nums border-primary/20 focus-visible:ring-primary/20 bg-background/50"
                      />
                    </div>
                  </div>
                  <Slider
                  ...
                  />
                  {/* Smart Ticks Hint */}
                  <div className="flex justify-between text-[10px] md:text-xs text-muted-foreground/50 px-1 font-medium uppercase tracking-wider">
                    <span>Cheap</span>
                    <span>Market</span>
                    <span>Premium</span>
                  </div>
                ...
                <div className="pt-4 md:pt-6 mt-4 md:mt-6 border-t border-border bg-muted/30 -mx-4 -mb-4 md:-mx-6 md:-mb-6 p-4 md:p-6">
                  <p className="text-xs md:text-sm text-muted-foreground mb-1 font-medium">Potential Monthly Capacity</p>
                  <div className="flex items-end gap-2 mb-3 md:mb-4">
                    <span
                      className={cn(
                        "text-3xl sm:text-5xl font-black text-foreground leading-tight transition-all duration-300",
                        animateResult && "scale-105"
                      )}
                    >
                      ₹{potentialEarnings.toLocaleString()}
                    </span>
                    <span className="text-muted-foreground font-bold mb-1 md:mb-1.5 text-[10px] md:text-sm uppercase tracking-wider">/ month</span>
                  </div>

                  {/* Embedded 'Claim' Action */}
                  <Link to="/seller/upload">
                    <Button className="w-full h-10 md:h-11 bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/20 group font-bold text-xs md:text-sm">
                      <Zap className="mr-2 h-3.5 w-3.5 md:h-4 md:w-4 fill-current" />
                      Claim This Income
                      <ArrowRight className="ml-2 h-3.5 w-3.5 md:h-4 md:w-4 transition-transform group-hover:translate-x-1" />
                    </Button>
                  </Link>

                  <p className="text-[10px] md:text-xs text-center text-muted-foreground mt-2 md:mt-3 font-medium opacity-70">
